        ...
        if photos_to_delete:
            ...
            for raw_id in photos_to_delete.split(','):
                try:
                    ...
                    continue
                photo = Photo.query.filter_by(id=photo_id, post_id=post.id).first()
                if photo:
            ...
            db.session.flush()
            # Garante que ainda existe uma foto principal
            remaining = Photo.query.filter_by(post_id=post.id).all()
